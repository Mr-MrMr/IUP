...
def transform_text(sample):
    ...
    splitted_sample = " ".join([x for x in splitted_sample if
...
for filename in os.listdir("SpamAssassin_datasets\\spam"):
    try:
        # Читаю файл с примером
        with open(f"SpamAssassin_datasets\\spam\\{filename}", "r") as f:
            sample = f.read()
        ...
        if body != None:
            ...
            body = [x for x in body if
                    re.search(r"[\s\S]*html|font|url|http|=|~|\\\[\s\S]*", x) == None]
            body = [w for w in body if not w in stop_words]
            # Если текст имеется, то записываю в датафрейм строку (не массив)
            if len(body) > 0:
                my_dataframe.loc[len(my_dataframe)] = [1, " ".join(body)]
    except UnicodeDecodeError:
        pass

for filename in os.listdir("SpamAssassin_datasets\\ham"):
    try:
        # Читаю файл с примером
        with open(f"SpamAssassin_datasets\\ham\\{filename}", "r") as f:
            sample = f.read()
        # Меняю неизвестные кодировки известной
        sample = sample.replace("charset=\"DEFAULT_CHARSET\"", "charset=\"Windows-1252\"")
        sample = sample.replace("charset=unknown-8bit", "charset=\"Windows-1252\"")
        ...
        if body != None:
            ...
            # Если текст имеется, то записываю в датафрейм строку (не массив)
            if len(body) > 0:
                my_dataframe.loc[len(my_dataframe)] = [0, " ".join(body)]
    except UnicodeDecodeError:
        pass
